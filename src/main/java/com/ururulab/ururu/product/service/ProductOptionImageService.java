...
public class ProductOptionImageService {
    ...
    public String uploadProductOptionImage(MultipartFile file) {
        ...
        try {
            PutObjectRequest putRequest = PutObjectRequest.builder()
                    .bucket(bucket)
                    .key("product-options/" + randomFilename) // 상품 옵션 전용 폴더
                    .contentType(file.getContentType())
                    .build();

            s3Client.putObject(putRequest, RequestBody.fromInputStream(file.getInputStream(), file.getSize()));
            log.info("Product option image uploaded: {}", randomFilename);
            ...
            log.error("S3 Error while uploading product option image: {}", e.awsErrorDetails().errorMessage());
            throw new RuntimeException("상품 옵션 이미지 업로드 중 S3 오류가 발생했습니다.", e);
        }

        return getFileUrl("product-options/" + randomFilename);
    }
    ...
    public void deleteProductOptionImage(String fileUrl) {
        ...
        try {
            ...
            log.error("S3 deletion failed for product option image: {}", e.awsErrorDetails().errorMessage());
    ...
    private boolean isDuplicate(MultipartFile multipartFile) {
        ...
        if (uploadedFileNames.contains(name) && uploadedFileSizes.contains(size)) return true;
        uploadedFileNames.add(name);
        uploadedFileSizes.add(size);
        return false;
    }
    ...
    public boolean doesImageExist(String fileUrl) {
        try {
            ...
            log.error("Error checking if image exists: {}", e.awsErrorDetails().errorMessage());
            return false;
        }
    }
}
